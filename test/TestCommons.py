...
class TestArgument(TestCase):
	def test_get_command(self):
		...
		arguments = Argument.get_command("-t", {"value": ["", 8, "", "test", ""], "input": True})
...
class TestScriptConfig(TestCase):
	def test_build_args(self):
		configuration = {"args": [{"name": "test", "javascript_section": "test"}], "test": {"abc": "test"}}
		...
		self.assertDictEqual(config.args[0].javascript_attributs, {"abc": "test"})
	...
	def test_build_scripts_from_configuration(self):
		...
		with patch.object(ScriptConfig, 'get_scripts_from_configuration') as config_mock:
		...
		self.assertListEqual(config_mock.mock_calls[0].args[0].ini_scripts_config, ["./unittests_configuration.ini"])
		self.assertListEqual(config_mock.mock_calls[0].args[0].json_scripts_config, ["./unittests_configuration.json"])

		self.assertEqual(len(config_mock.mock_calls[1].args), 2)
		self.assertDictEqual(config_mock.mock_calls[1].args[0].scripts, {"test.py": "test"})
		self.assertDictEqual(config_mock.mock_calls[1].args[0].test, {})

		self.assertEqual(len(config_mock.mock_calls[2].args), 2)
		self.assertDictEqual(config_mock.mock_calls[2].args[0].scripts, {"test.sh": "test"})
	...
	def test_get_scripts_from_configuration(self):
		...
		with patch.object(ScriptConfig, "get_script_path") as mock_config_path:
			...
			script_configs = ScriptConfig.get_scripts_from_configuration(configuration, configuration)
		...
		with patch.object(WebScripts.commons, "get_real_path") as mock_real_path:
			mock_real_path.return_value = "/fake/path/test.py"
			with self.assertRaises(WebScriptsConfigurationError):
				ScriptConfig.get_scripts_from_configuration(configuration, configuration)
	...
	def test_get_Windows_default_script_launcher(self):
		with patch.object(WebScripts.commons, "system") as mock_system:
			...
			self.assertIsNone(ScriptConfig.get_Windows_default_script_launcher(None))

		with patch.object(WebScripts.commons, "system") as mock_system:
			mock_system.return_value = "Windows"

			with self.assertRaises(WebScriptsSecurityError):
				ScriptConfig.get_Windows_default_script_launcher({"path": "abc.command injection", "name": ""})

			with patch.object(WebScripts.commons, "Popen") as mock_process:
				mock_process.return_value = Mock(returncode=1, communicate=Mock(return_value=("", "")))
				self.assertIsNone(ScriptConfig.get_Windows_default_script_launcher({"path": "abc.py", "name": ""}))

				mock_process.return_value = Mock(returncode=0, communicate=Mock(return_value=("", "")))
				self.assertIsNone(ScriptConfig.get_Windows_default_script_launcher({"path": "abc.py", "name": ""}))

				mock_process.return_value = Mock(returncode=0, communicate=Mock(return_value=('="abc', "")))
				self.assertEqual(ScriptConfig.get_Windows_default_script_launcher({"path": "abc.py", "name": ""}), "abc")
	...
	def test_get_script_path(self):
		...
		script_path = ScriptConfig.get_script_path(server_config, script_config)
	...
	def test_get_documentation_from_configuration(self):
		self.assertEqual(ScriptConfig.get_documentation_from_configuration({"documentation_file": "test"}, "test.py", ["."]), "test")
		self.assertIsNone(ScriptConfig.get_documentation_from_configuration({}, "test_doc.py", ["."]))

		with open("test_doc.html", "w") as file:
			file.write("<html></html>")

		self.assertEqual(ScriptConfig.get_documentation_from_configuration({}, "test_doc.py", ["."]), path.join(".", "test_doc.html"))
		os.remove("test_doc.html")

	def test_get_arguments_from_config(self):
		self.assertListEqual(ScriptConfig.get_arguments_from_config(None, None), [])

		with self.assertRaises(WebScriptsConfigurationError):
			ScriptConfig.get_arguments_from_config("test", {})

		with self.assertRaises(WebScriptsConfigurationError):
			ScriptConfig.get_arguments_from_config("test", {"test": {"arg": "arg_test"}})

		args = ScriptConfig.get_arguments_from_config("test", {"test": {"arg": "arg_test"}, "arg_test": {}})
		self.assertEqual(len(args), 1)
		self.assertDictEqual(args[0], {"name": "arg"})

	def test_get_script_config_from_specific_file_config(self):
		self.assertEqual(ScriptConfig.get_script_config_from_specific_file_config({}, None), (None, {}))

		with open("test.json", "w") as file:
			json.dump({"no_script": {"test": "test"}}, file)

		with self.assertRaises(WebScriptsConfigurationError):
			ScriptConfig.get_script_config_from_specific_file_config({"configuration_file": "test.json"}, {})

		with open("test.ini", "w") as file:
			file.write("[script]\ntest=test")
		
		configs = ScriptConfig.get_script_config_from_specific_file_config({"configuration_file": "test.ini"}, {})
		...
		configs = ScriptConfig.get_script_config_from_specific_file_config({"configuration_file": "test.json"}, {})
	...
	def test_get_docfile_from_configuration(self):
		...
		with patch.object(WebScripts.commons, "iglob") as mock_glob:
			mock_glob.return_value = ["error.html", "no_test.html", "te.html", "test.html"]
			self.assertEqual(ScriptConfig.get_docfile_from_configuration(config, "test.py"), "test.html")

class TestCallableFile(TestCase):

	def test___call__(self):
		with patch.object(WebScripts.commons, "get_file_content") as mock_file_content:
			...
			self.assertDictEqual(headers, {"Content-Type": "text/javascript; charset=utf-8"})

			for headers_ref, extentions in [
				({"Content-Type": "text/html; charset=utf-8"}, (".html", ".htm", ".shtml", ".xhtml")),
				...
				({"Content-Type": "application/pdf; charset=utf-8"}, (".pdf",)),
            ...
            ".opml",)),
				...
				({"Content-Type": "application/octet-stream"}, (".test",".error",".bin")),
			]:
				for extention in extentions:
					file = CallableFile("static", f"file.{extention}", f"file.{extention}")
					code, headers, content = file(None)
					self.assertEqual(code, "200 OK")
			...
			CallableFile.template_script = "%(name)s_%(user)s_%(csrf)s_%(nonce)s"

			file = CallableFile("script", "file.py", "file.py")
			code, headers, content = file(user)
			self.assertEqual(code, "200 OK")
			self.assertEqual(headers["Content-Type"], "text/html; charset=utf-8")
			self.assertRegex(headers["Content-Security-Policy"], r"default-src 'self'; form-action 'none'; frame-ancestors 'none'; script-src 'self' 'nonce-[\da-fA-F]{20}")
			self.assertRegex(content, r"file\.py_test_[\w\d+/]+={0,2}_[\da-fA-F]{20}")
...
class TestBlacklist(TestCase):
	...
	def test___str__(self):
		blacklist = Blacklist(None, None)
		self.assertEqual("Blacklist(counter=1, blacklist_time=0.0)", str(blacklist))
...
class TestSession(TestCase):
	...
	def test___str__(self):
		self.assertEqual(f"Session(Time={time() - self.session.time}, IP=127.0.0.1, Cookie={self.session.cookie}, User=DefaultNamespace(__required__=[], __optional__=[], __default__={{}}, __types__={{}}, id=0))", str(self.session))
	...
	def test_check_session(self):
		...
		self.assertEqual(Session.check_session("SessionID=abc:", None, None, 0), 0)
		self.assertEqual(Session.check_session("SessionID=1:", self.pages, None, 0), 0)
		self.assertEqual(Session.check_session("SessionID=0:", self.pages, "", 0), 0)
		self.assertEqual(Session.check_session("SessionID=0:", self.pages, self.ip, 0), 0)
		self.assertEqual(Session.check_session(f"SessionID=0:{session.cookie}", self.pages, self.ip, 0), self.user)
		session.time = 0
		self.assertEqual(Session.check_session(f"SessionID=0:{session.cookie}", self.pages, self.ip, 0), 0)
